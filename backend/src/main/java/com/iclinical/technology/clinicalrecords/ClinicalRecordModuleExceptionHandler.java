...
@RestControllerAdvice(basePackages = "com.iclinical.technology.clinicalrecords")
class ClinicalRecordModuleExceptionHandler {

    @ExceptionHandler(ClinicalRecordNotFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    ApiError handleNotFound(ClinicalRecordNotFoundException exception) {
        return new ApiError(exception.getMessage(), Map.of());
    }

    @ExceptionHandler(ClinicalRecordValidationException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    ApiError handleValidation(ClinicalRecordValidationException exception) {
        return new ApiError(exception.getMessage(), Map.of());
    }
