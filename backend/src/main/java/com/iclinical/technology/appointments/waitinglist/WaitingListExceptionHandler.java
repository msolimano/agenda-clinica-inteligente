...
@RestControllerAdvice(basePackages = "com.iclinical.technology.appointments.waitinglist")
class WaitingListExceptionHandler {

    @ExceptionHandler(WaitingListNotFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    ApiError handleNotFound(WaitingListNotFoundException exception) {
        return new ApiError(exception.getMessage(), Map.of());
    }

    @ExceptionHandler(WaitingListValidationException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    ApiError handleValidation(WaitingListValidationException exception) {
        return new ApiError(exception.getMessage(), Map.of());
    }
